...
## 24.10 Real provider adapters cover `chat`/`embed`, not the full capability surface

`crates/ulx-runtime/src/provider/` ships adapters for OpenAI-compatible servers (OpenAI, Groq, vLLM, LM Studio, and any other server speaking the same `/chat/completions` shape), Anthropic, Gemini, Cohere, and Ollama — but only for the `chat` capability (plus `embed` where the vendor has a simple, stable endpoint). `vision`, `transcribe`, `speak`, and `generate_image` remain `MockProvider`-only: each is a genuinely different, often multipart/binary API per vendor, and building all four across six vendor families is real, separate work this pass didn't take on. Retry behavior is deliberately a single retry on HTTP 429/5xx with a fixed backoff (`provider::transport::send_json_with_retry`), not the exponential-backoff-with-jitter-and-circuit-breaker a production-grade client would eventually want. Refusal detection (mapped to `Draft<T>`'s `Unsettled(Refused(..))`, §9.3) relies on each vendor's specific finish/stop-reason field and will miss a vendor that signals refusal a different way (Cohere's Chat v2 API, for instance, exposes no such signal at all, so `CohereProvider` never produces `Refused`). None of this is hidden behind a false "full parity" claim — it's the same honest-accounting posture as §24.4's capability-negotiation gap, just one layer further down the stack.
