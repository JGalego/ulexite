...
`crates/ulx-runtime/src/provider/` ships adapters for OpenAI-compatible servers (OpenAI, Groq, vLLM, LM Studio, and any other server speaking the same `/chat/completions` shape), Anthropic, Gemini, Cohere, and Ollama, covering `chat` (all), `embed` (`openai_compatible`/`gemini`/`cohere`/`ollama`), `vision` (`openai_compatible`/`anthropic`/`gemini`/`ollama`), and — `openai_compatible` only — `transcribe`/`speak`/`generate_image`. The gaps that remain are real, not hidden:
...
- **`vision` is images only.** jpg/png/gif/webp, read locally or passed through as an `http(s)://`/`data:` URI where the vendor's API fetches URLs directly (OpenAI-compatible, Anthropic). Gemini and Ollama only accept a local file today (Gemini's URL-fetching path requires its separate File-API upload flow, not implemented; Ollama's native API has no URL-fetch concept at all). `ArtifactType::Pdf`/`Video` are accepted by `ulx-sema`'s capability check (§9.6/§24.4) but any real vendor call with one fails at the HTTP boundary with a plain error, not a type-checked rejection — the same "declared capability vs. what a plugin actually does" gap §24.4 already names.
- **`transcribe`/`speak`/`generate_image` are `openai_compatible`-only.** Anthropic and Cohere don't expose these APIs at all; Gemini and a native Ollama server do have their own equivalents, unimplemented here. A user who wants these from another vendor is stuck with `openai_compatible` (OpenAI itself, or Groq for `transcribe`) until a future adapter is written.
