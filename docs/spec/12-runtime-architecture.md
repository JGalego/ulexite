...
A provider adapter is a plugin implementing the capability trait from §9.6: it declares which capabilities (`chat`, `vision`, `embed`, ...) it satisfies, at which guarantee tier (`guaranteed`/`negotiated`/`unsupported` structured output, §9.6), and its cost/latency metadata. The runtime's provider registry resolves an unqualified `ask <capability>(...)` call against every provider registered for it — with exactly one match, that's the whole policy; with zero, a clear `UnknownCapability` error; with two or more and nothing disambiguating, a clear `Ambiguous` error naming every candidate, never a silent first-match (v0.1 doesn't yet implement the cost-ceiling/latency-budget/circuit-breaker-state policy this paragraph originally envisioned recomputing per call — only explicit disambiguation exists today). Registering a new provider is implementing the trait and adding it to the registry; no compiler, grammar, or IR change is required — directly satisfying the mission's provider-independence requirement and avoiding the fate of every framework in §2.3, where "provider-agnostic" meant a hand-maintained adapter (LiteLLM, `init_chat_model`, SK connectors) with silently uneven feature parity.

Two ways to disambiguate, or to configure a provider at all: the manifest (`ulexite.toml`'s `[providers.<name>]` tables, §14.1), or a `provider <name> [from "<manifest-entry>"] { ... }` declaration directly in `.ulx` source — the one deliberate, explicit exception to "the runtime never names a vendor" above, since a standalone `provider` block (no `from`) does name one, in source. It exists for the case a `ulexite.toml` doesn't cover: a project that wants its provider config to travel with the source file rather than live in a separate manifest, or that has none at all. A `from`-inheriting block instead overlays its own fields on top of a manifest entry's, field-by-field and capability-by-capability. Either way, an `ask` call site itself is unaffected — it still only ever names a capability, plus optionally a provider *name* (`ask chat(provider: "name")`, resolved against whichever `provider` decls and manifest entries are in scope), never a vendor kind directly. The CLI's `--provider name` flag (repeatable) does the same selection at the whole-run level instead of per call. See `crates/ulx-cli/src/providers.rs` for the merge/precedence rules (a `.ulx` decl's name colliding with an unrelated manifest entry is a hard error, not a silent pick) and README.md's "Configuring providers" for the user-facing walkthrough.
