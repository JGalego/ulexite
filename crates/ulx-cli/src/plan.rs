...
"#;
    ...
    fn mock_registry_and_infos() -> (ProviderRegistry, BTreeMap<String, ProviderInfo>) {
        ...
        registry.register(
            ...
            build_provider(
                &spec,
                std::path::Path::new("/tmp/ulexite-plan-test-artifacts"),
            )
            .unwrap(),
        );
    ...
    fn plans_translate_examples_chat_and_judge_calls() {
        ...
        assert_eq!(
            judge_rows.len(),
            1,
            "expected one judge call site: {rows:?}"
        );
        ...
        for row in chat_rows.iter().chain(judge_rows.iter()) {
            let target = row
                .resolution
                .as_ref()
                .unwrap_or_else(|e| panic!("expected `{}` to resolve, got error: {e}", row.label));
    ...
    fn known_arg_replaces_the_generic_unknown_allowance() {
        ...
        let rows_unknown =
            build_plan(&ir, "Translate", &registry, &infos, &BTreeMap::new()).unwrap();
