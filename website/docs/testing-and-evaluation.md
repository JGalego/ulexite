...
`expect ... satisfies <judge>` grades the subject against a judge's rubric — the design intends this to resample/re-evaluate up to a configurable retry budget until the verdict converges, an auto-waiting assertion adapted to LLM non-determinism rather than grading a single sample once. **This resample loop is real today, with one narrowing: the retry budget is a fixed 3 attempts, not a per-statement configurable one** (there's no grammar yet for writing a custom budget). Each resample attempt is a genuinely fresh judge call — a real provider actually gets re-invoked, it isn't the same cached verdict replayed three times — and the loop stops the moment a verdict passes. If none of the 3 attempts pass, the check fails with the last verdict's message, noting how many attempts were made. `expect ... settles within duration(...)` (the tool-call/async-effect analogue) is described in the full design but has no working implementation — there's no async/tool-call machinery in the runtime at all yet.
